...
public class Node3 extends Thread
{
		...
		public static final String node_id = "P2";
		...
		public Node3() {
			System.out.println("Process: " + Node3.node_id);
			vector_time = new int[num_nodes];
			Arrays.fill(vector_time, 0);
		}
		
		public static void main(String[] args) throws UnknownHostException, IOException, InterruptedException {
			Node3 Node3_thread = new Node3();
			Node3_thread.initialize();
			if(Node3.link_thread != null){
				Node3.link_thread.join();
			}
		}
		...
		public void initialize() throws UnknownHostException, IOException, InterruptedException{
			...
			link_thread.start();
			StringBuilder vectorTime_initial = display(Node3.vector_time);
		...
		public void vectorTime_Order() {
				...
				for(int i=0; i < n; i++){
					...
					for(int j=0; j <= k ; j++){
						if(j == k){ break; }
						if(event_timeVector[event_pid] != (Node3.vector_time[event_pid] + 1)){ break; }
						if((j != event_pid) && event_timeVector[j] > Node3.vector_time[j]){ break; }
						if(j == (k - 1)){
							int[] past_vt = new int[Node3.vector_time.length];
							past_vt = Arrays.copyOf(Node3.vector_time, past_vt.length);
							Node3.vector_time[event_pid] = Math.max(Node3.vector_time[event_pid], event_timeVector[event_pid]);
							recvd = true;
							fncall_recv(past_vt); }
					}
