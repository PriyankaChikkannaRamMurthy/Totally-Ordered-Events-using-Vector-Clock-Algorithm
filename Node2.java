...
public class Node2 extends Thread
{
		...
		public static final String node_id = "P1";
		...
		public Node2() {
			System.out.println("Process: " + Node2.node_id);
			vector_time = new int[num_nodes];
			Arrays.fill(vector_time, 0);
		}
		
		public static void main(String[] args) throws UnknownHostException, IOException, InterruptedException {
			Node2 Node2_thread = new Node2();
			Node2_thread.initialize();
			if(Node2.link_thread != null){
				Node2.link_thread.join();
			}
		}
		...
		public void initialize() throws UnknownHostException, IOException, InterruptedException{
			...
			link_thread.start();
			StringBuilder vectorTime_initial = display(Node2.vector_time);
		...
		public void vectorTime_Order() {
				...
				for(int i=0; i < n; i++){
					...
					for(int j=0; j <= k ; j++){
						if(j == k){ break; }
						if(event_timeVector[event_pid] != (Node2.vector_time[event_pid] + 1)){ break; }
						if((j != event_pid) && event_timeVector[j] > Node2.vector_time[j]){ break; }
						if(j == (k - 1)){
							int[] past_vt = new int[Node2.vector_time.length];
							past_vt = Arrays.copyOf(Node2.vector_time, past_vt.length);
							Node2.vector_time[event_pid] = Math.max(Node2.vector_time[event_pid], event_timeVector[event_pid]);
							recvd = true;
							fncall_recv(past_vt); }
					}
